...
$$
\begin{aligned}
P(X = 0) & = \underset{每次都未抽中}{(1 - p)^n} \\
P(X = n) & = \underset{每次都抽中}{p^n} \\
\ \\
P(X = 1) & = \underset{一次抽中}{p} \underset{其余均为抽中}{(1 - p)^{n - 1}} \underset{1次不定序}{n} = np(1 - p)^{n - 1}
\ \\
P(X = k) & = \underset{k次抽中}{p^k} \underset{其余均为抽中}{(1 - p)^{n - k}} \underset{k次均不定序}{C_n^k} = C_n^k p^k (1 - p)^{n - k}
\end{aligned}
$$
...
E(X)
& = \sum_{i = 1}^n i P(X = i) \\
& = \sum_{i = 1}^n i C_n^i p^i (1 - p)^{n - i} \\
& = C_n^0 (1 - p)^n + C_n^1 p(1 - p)^{n - 1} + C_n^2 p^2 (1 - p)^{n - 2} + \cdots + C_n^n p^n \\
& = \frac{n!}{n!} (1 - p)^n + \frac{n!}{(n - 1)!} p(1 - p)^{n - 1} + \frac{n!}{2! (n - 2)!} p^2 (1 - p)^{n - 2} + \cdots + \frac{n!}{n!} p^n \\
& = np ()
\end{aligned}
$$
